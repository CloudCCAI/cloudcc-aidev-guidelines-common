...
echo "8. Before async development or CI merge, run: python3 \"${ROOT_DIR}/scripts/check-assignment.py\" \"${STATE_DIR_PATH}\" --developer DEV-xxx --task TASK-xxx --files path/to/file"
echo "9. For a GitHub PR gate example, copy templates/github-workflows/check-assignment.yml into .github/workflows/"
echo "10. For manager team status, run: python3 \"${ROOT_DIR}/scripts/summarize-team-status.py\" \"${STATE_DIR_PATH}\" --write"
echo "11. Run validation: python3 \"${ROOT_DIR}/scripts/validate-state.py\" \"${STATE_DIR_PATH}\""
