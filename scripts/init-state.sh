...
echo "10. For Codeup change requests, read templates/platforms/codeup/ and configure local YUNXIAO_TOKEN"
echo "11. For test-environment pushes, run: python3 \"${ROOT_DIR}/scripts/push-test-environment.py\" --source-branch feat/TASK-xxx --target-branch dev"
echo "12. For a GitHub PR gate example, copy templates/github-workflows/check-assignment.yml into .github/workflows/"
echo "13. For manager team status, run: python3 \"${ROOT_DIR}/scripts/summarize-team-status.py\" \"${STATE_DIR_PATH}\" --write"
echo "14. Run validation: python3 \"${ROOT_DIR}/scripts/validate-state.py\" \"${STATE_DIR_PATH}\""
